...
public class RaidCommissionLegendaryItem extends Item {
    ...
    int baseReward = 800000; // Base value used to calculate rewards. an Easy raid with no bad_omens.
    float modMult = 1.3f;   // Reward multiplier for every bad_omen present.
    float goodModMult = 0.8f;// Reward multiplier for every good_omen present.
    float difMult = 1.7f;   // Reward multiplier for every difficulty level above easy.
    ...
    public void appendHoverText(ItemStack stack, Level worldIn, List<Component> tooltip, TooltipFlag flagIn) {
        int difficulty =-1; // Difficulty saved as int, (0,1,2),(Legendary,Legendary+,Legendary++)
        ...
        if(Screen.hasControlDown()) {
            switch(difficulty)
            {
                ...
                case 1:
                    ...
                    break;
                case 2:
                    tooltip.add(new TextComponent("§c§oImpossible (☠ ☠ ☠ ☠ ☠ ☠)§r"));
                    break;
                default:
                    tooltip.add(new TextComponent("§4§o§k?a?b?c?§r"));
                    break;
            }
        }else{ // Hide difficulty name when ctrl not pressed
            switch(difficulty)
            {
                case 0:
                    tooltip.add(new TextComponent("§d§o              ☠ ☠ ☠ ☠§r"));
                    break;
                case 1:
                    tooltip.add(new TextComponent("§d§o             ☠ ☠ ☠ ☠ ☠§r"));
                    break;
                case 2:
                    tooltip.add(new TextComponent("§c§o           ☠ ☠ ☠ ☠ ☠ ☠§r"));
                    break;
                default:
                    tooltip.add(new TextComponent("§4§o§k?a?b?c?§r"));
                    break;
    ...
    public void inventoryTick(ItemStack stack, Level level, Entity entity, int p_41407_, boolean p_41408_) {
        ...
        if(!level.isClientSide){
            if(stack.getTag() == null){
                ...
                int good_omen1 = randomInt(-3,3);
                int good_omen2 = randomInt(-3,3);
                int good_omen3 = randomInt(-3,3);
                int bad_omen1 = randomInt(-3,3); // 0 is the first bad_omen, but we make the range wider such that there
                int bad_omen2 = randomInt(-3,3); // is a lower chance of there being a bad_omen in each slot. Anything
                int bad_omen3 = randomInt(-3,3); // less than 0 is treated as no bad_omen, so this works.
