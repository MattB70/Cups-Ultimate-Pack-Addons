...
/*
    ...
    */
    ...
    private <P extends IAnimatable> PlayState predicate(AnimationEvent<P> event) {
        // Set an animation here, even though it won't play due to playstate set to STOP initially. This will stop a null
        // reference crash. GeckoLib items are a bit janky, but what isn't? :)
        event.getController().setAnimation(new AnimationBuilder().addAnimation(useAnimationName, ILoopType.EDefaultLoopTypes.PLAY_ONCE));
    ...
    public void onAnimationSync(int id, int state) {
        if (state == ANIM_OPEN) {
            ...
            if (controller.getAnimationState() == AnimationState.Stopped) {
                final LocalPlayer player = Minecraft.getInstance().player;
                if (player != null) {
                    player.displayClientMessage(new TextComponent("Activating Item").withStyle(ChatFormatting.BLUE,ChatFormatting.ITALIC), true);
                }
                // If you don't do this, the popup animation will only play once because the
                // animation will be cached.
                controller.markNeedsReload();
